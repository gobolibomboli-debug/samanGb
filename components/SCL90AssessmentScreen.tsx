...
import { SCL90_QUESTIONS } from '../services/scl90Service';
import { useLanguage } from '../contexts/LanguageContext';
import JourneyProgress from './JourneyProgress';
import { SCL90AssessmentState } from '../types';

interface SCL90AssessmentScreenProps {
  assessmentState: SCL90AssessmentState;
  onAnswer: (questionId: number, score: number) => void;
  onBack: () => void;
  onFinishRandomly: () => void;
}

const SCL90AssessmentScreen: React.FC<SCL90AssessmentScreenProps> = ({ assessmentState, onAnswer, onBack, onFinishRandomly }) => {
  const { currentQuestionIndex } = assessmentState;
  const [isFading, setIsFading] = useState(false);
  const { t } = useLanguage();
  
  const totalQuestions = SCL90_QUESTIONS.length;
  const currentQuestion = SCL90_QUESTIONS[currentQuestionIndex];
  ...
  const likertOptions = [
    { labelKey: 'scl90_likert_0', value: 0 },
    { labelKey: 'scl90_likert_1', value: 1 },
    { labelKey: 'scl90_likert_2', value: 2 },
    { labelKey: 'scl90_likert_3', value: 3 },
    { labelKey: 'scl90_likert_4', value: 4 },
  ];
            ...
            <div className="flex flex-col sm:flex-row justify-center items-center gap-2 sm:gap-4 my-8">
              {likertOptions.map(({ labelKey, value }) => (
                <button
                  ...
                  className="group w-full sm:w-auto flex-grow sm:flex-grow-0 sm:w-28 h-20 sm:h-28 p-2 flex flex-col items-center justify-center text-center bg-black/20 backdrop-blur-md border-2 border-sky-400/20 rounded-xl shadow-lg transition-all duration-300 ease-in-out transform hover:scale-105 hover:border-sky-400 hover:bg-sky-900/20 active:scale-95"
                >
...
export default SCL90AssessmentScreen;
